...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # END Problem 8b
            #self视作放进来的ant
            if place.ant.is_container and place.ant.can_contain(self):#如果原来的位置是container
                place.ant.store_ant(self)
            elif self.is_container and self.can_contain(place.ant):#如果放进来的是container
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        curr = self.place
        distance = 0
        while curr.entrance is not None:
            if not curr.is_hive:
                if len(curr.bees) >0 and self.lower_bound<= distance <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        curr = self.place
        bee_list = curr.bees
        def bee_health_reduce(count=amount):
            for bee in bee_list[:]:#加上了[:]就代表只是原来list的一个复制了
                bee.reduce_health(count)
        bee_health_reduce(amount)
        if self.health <= amount:
            bee_health_reduce(self.damage)
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if len(self.place.bees)>0 and self.turns_to_chew ==0:
            self.turns_to_chew = self.chewing_turns
            eaten_bee_list = self.place.bees
            eaten_bee = random_bee(eaten_bee_list)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def construct(cls, gamestate):
        ...
        if cls.food_cost > gamestate.food:
            return
        if not gamestate.already_has_a_Queen:
            gamestate.already_has_a_Queen = True
            return super().construct(cls, gamestate)
        else:
            return None
        # END Problem 12
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, food=2):
        ...
        # BEGIN Problem 12
        self.already_has_a_Queen = False
        # END Problem 12
