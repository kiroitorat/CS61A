...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_M = length(left(m))* total_weight(left_end)
        right_M = length(right(m))* total_weight(right_end)
        
    return left_M == right_M and balanced(left_end) and balanced(right_end)
...
def totals_tree(m):
    ...
    """
    if is_planet(m):
        return tree(mass(m))#如果是行星，则返回一个树叶，lable是质量
    else:
        branches = [totals_tree(end(f(m))) for f in [left, right]] #如果是mobile，考虑它的分支有两个
...
def sub_interval(x, y):
    ...
    and any value in y."""
    lower_ = lower_bound(x)-upper_bound(y)
    upper_ = upper_bound(x)-lower_bound(y)
    return interval(min(lower_, upper_), max(lower_, upper_))
