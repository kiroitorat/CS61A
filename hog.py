...
def tail_points(opponent_score):
    ...
    取模100得到的就是后两位数,对10就是后一位数,依次类推
    地除10得到的是前两位数,地除100就是前一位数
    995///10 = 99
    995//100 = 9
    ...
    # BEGIN PROBLEM 2
    # ge , shi = 0 , 0
    # if opponent_score // 100 == 0 :
    #     ge , shi = opponent_score % 10 , opponent_score // 10
    #     return 2 * abs(shi - ge) + 1
    # ge ,shi = opponent_score % 100 % 10, opponent_score // 10 % 10
    # return 2 * abs(ge - shi) + 1
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal :
        ...
        '''
        if who == 0:
            curr = strategy0(score0 , score1)
            score0 = update(curr , score0, score1 , dice)
        elif who == 1:
            curr = strategy1(score1 , score0)
            score1 = update(curr , score1, score0 , dice)
        who=1-who
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    begin_num_rolls= strategy(0,0)
    have_different= False
    for m in range(goal):
        for n in range(goal):
            curr_num_roll = strategy(m ,n)
            if begin_num_rolls != curr_num_roll:
                have_different = True
    if have_different :
        return False
    return True
...
def make_averaged(original_function, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def original_function_inside(*args):
        sum = 0
        for i in range(total_samples):
            sum += original_function(*args)
        return sum / total_samples
    return original_function_inside
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for i in range(1,11):
        ...
        #不能把MA直接assign给avg然后跟largest比，因为MA返回的是函数
        avg=average(i, dice) 
        if largest<avg:
            largest=avg
            marker=i
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    pig_tail_points = tail_points(opponent_score)
    if pig_tail_points >= threshold:
        return 0
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    curr = square_update(0, score, opponent_score)
